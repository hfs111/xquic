...
#include "rebind.h"
 ...
 //hfs:临时定义
 int connection_migration_enabled = 0;
 ...
 typedef struct user_conn_s {
     ...
     int                 tracked_pkt_cnt;
     int                 is_reconnecting; //hfs: add to indicate whether the connection is in the process of reconnecting
 } user_conn_t;
 ...
 */
 ...
 {
     int ip_type = (g_ipv6 ? AF_INET6 : AF_INET);
     user_conn->fd = xqc_client_create_socket(ip_type,  //hfs: change NULL to enp6s19
                                              user_conn->peer_addr, user_conn->peer_addrlen, g_multi_interface[0]);
     if (user_conn->fd < 0) {
         printf("xqc_create_socket error\n");
         return -1;
     }
 ...
 xqc_client_h3_conn_close_notify(xqc_h3_conn_t *conn, const xqc_cid_t *cid, void *user_data)
 {
     ...
     // if(user_conn->is_reconnecting){
     //     printf("[APP] conn_close at %ld\n", xqc_now());
     //     on_enp6s19_ip_reconnect(user_conn); //hfs: 如果是重连，重新连接
     //     user_conn->is_reconnecting = 0; 
     //     printf("[info] h3_conn is reconnecting, skip close notify\n");
     //     return 0; //hfs: 如果是重连，直接返回
     // }
 ...
 xqc_client_h3_conn_handshake_finished(xqc_h3_conn_t *h3_conn, void *user_data)
 {
     ...
     printf("[APP] handshake_finished at %ld\n", xqc_now());
         //hfs: 如果是重连，自动发送请求
     if (user_conn->is_reconnecting > 0) {
         printf("[INFO] h3_conn create after reconnect, auto send request\n");
         //user_conn->is_reconnecting = 0; // 重置标志
         ...
         printf("[INFO] xqc_h3_request_create success\n");
 ...
 size_t read_offset() {
     ...
     if (file) {
         fscanf(file, "%zu", &offset);
         fclose(file);
     } else {
         printf("Failed to open offset file for reading");
     }
     return offset;
 }
 
 void save_offset(size_t offset) {
     FILE *file = fopen("offset.txt", "w");
     if (file) {
         fprintf(file, "%zu\n", offset);
         fclose(file);
     } else {
         perror("Failed to open offset file for writing");
     }
 }
 ...
 int xqc_client_request_send(xqc_h3_request_t *h3_request, user_stream_t *user_stream) {
     ...
     if (user_stream->send_body == NULL) {
         ...
         fseek(fp, 0, SEEK_END);
         user_stream->send_body_len = ftell(fp);
         fseek(fp, 0, SEEK_SET);
         user_stream->send_body = malloc(user_stream->send_body_len);
         if (fread(user_stream->send_body, 1, user_stream->send_body_len, fp) != user_stream->send_body_len) {
     ...
     xqc_http_header_t header[] = {
         { .name = { .iov_base = ":method", .iov_len = 7 }, .value = { .iov_base = "POST", .iov_len = 4 }, .flags = 0 },
         { .name = { .iov_base = ":scheme", .iov_len = 7 }, .value = { .iov_base = "https", .iov_len = 5 }, .flags = 0 },
         { .name = { .iov_base = "host", .iov_len = 4 }, .value = { .iov_base = g_host, .iov_len = strlen(g_host) }, .flags = 0 },
         { .name = { .iov_base = ":path", .iov_len = 5 }, .value = { .iov_base = "/upload", .iov_len = 7 }, .flags = 0 },
         //{ .name = { .iov_base = "content-type", .iov_len = 12 }, .value = { .iov_base = "video/mp4", .iov_len = 10 }, .flags = 0 },
         { .name = { .iov_base = "test-type", .iov_len = 12 }, .value = { .iov_base = g_test_type, .iov_len = 20}, .flags = 0 },
         { .name = { .iov_base = "content-length", .iov_len = 14 }, .value = { .iov_base = content_len, .iov_len = strlen(content_len) }, .flags = 0 },
     };
 
     xqc_http_headers_t headers = { .headers = header, .count = 6 };
     ...
     if (user_stream->send_offset < user_stream->send_body_len) {
         ...
         if(user_stream->send_body_len - user_stream->send_offset < max_send_size)
         ...
         user_stream->send_offset += ret;
         printf("累计发送字节数: %zu\n", user_stream->send_offset);
         save_offset(user_stream->send_offset);
     }
 ...
 xqc_client_request_write_notify(xqc_h3_request_t *h3_request, void *user_data)
 {
     ...
     printf("request write notify!:%"PRIu64"\n", xqc_h3_stream_id(h3_request));
         ...
         // uint64_t cwnd = h3_request->h3_stream->h3c->conn->conn_settings.cc_params.init_cwnd;
         // printf("Current cwnd = %llu bytes\n", (unsigned long long)cwnd);
 ...
 void reset_to_default(user_conn_t *user_conn) {
 ...
 int main(int argc, char *argv[]) {
     ...
     while ((ch = getopt_long(argc, argv, "a:p:P:n:c:Ct:T:1s:w:r:l:Ed:u:H:h:Gx:6NMR:i:V:v:q:o:fe:F:D:b:B:J:Q:U:AyzS:gm:Y:j", long_opts, NULL)) != -1) {
         ...
         case 'j':
             ...
             break;
         case 'Y': //hfs:测试类型
             printf("option test-type: %s\n", optarg);
             snprintf(g_test_type, sizeof(g_test_type), optarg);
             break;
         case 'm':
             connection_migration_enabled = 1;
             printf("Connection migration enabled.\n");
             break;
         case 'U':
