 ...
 */
 ...
 {
     // 假设 user_data 中包含了 offset 信息
     user_stream_t *user_stream = (user_stream_t *)user_data;
 
     // 打印已确认的 offset 值
     printf("已确认的 offset: %zu\n", user_stream->send_offset);
 ...
 xqc_server_request_read_notify(xqc_h3_request_t *h3_request, xqc_request_notify_flag_t flag, void *user_data)
 {
     ...
     // 记录开始时间
     static xqc_usec_t start_time = 0; // 静态变量用于记录第一次执行的时间
     if (start_time == 0) {
         start_time = xqc_now(); // 记录第一次执行的时间
     }
     ...
     if (flag & XQC_REQ_NOTIFY_READ_BODY) {
         ...
         //hfs: 记录服务端offset，用于断点续传
         global_saved_offset = user_stream->recv_body_len; // 更新全局变量
         // mpshell
             // 打印源 IP 信息
     if (h3_request->h3_stream->h3c->conn) { // 假设 h3_request 有一个 conn 字段
         struct sockaddr *peer_addr = (struct sockaddr *)h3_request->h3_stream->h3c->conn->peer_addr; // 获取源 IP 地址
         char ip_str[INET6_ADDRSTRLEN]; // 用于存储 IP 字符串
         ...
         xqc_list_for_each_safe(pos, next, &h3_request->h3_stream->h3c->conn->conn_paths_list) {
         ...
         if(g_debug)
              printf("xqc_h3_request_recv_body read:%zd, offset:%zu, fin:%d, ip:%s\n", read_sum, user_stream->recv_body_len, fin, ip_str);
     }
     ...
     if (fin) {
         ...
         xqc_server_request_send(h3_request, user_stream);
         start_time = 0; // 重置开始时间，以便下次调用时重新计时
     }
 ...
 int main(int argc, char *argv[]) {
     ...
     while ((ch = getopt_long(argc, argv, "a:p:dDefc:Cs:w:r:l:u:x:6bS:MR:o:EK:mLQ:U:yH", long_opts, NULL)) != -1) {
         ...
         case 'd':
             ...
             g_debug = 1;
             break;
         //hfs: for delay validate PATH_CHALLENGE and PATH_RESPONSE
         case 'D':
             printf("option hfs: delay validate PATH_RESPONSE\n");
             g_delay_challenge = 1;
             break;
         case 'H':
     ...
     if (g_test_case == 1 || g_test_case == 5 || g_test_case == 6 || g_sid_len != 0) {
         ...
         //hfs: set delay_challenge flag for engine's config
     if(g_delay_challenge)
